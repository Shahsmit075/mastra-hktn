...
const sdk = new NodeSDK({
  ...
  instrumentations: [
    getNodeAutoInstrumentations({
      ...
      '@opentelemetry/instrumentation-http': { enabled: false }, // Disable to prevent native fetch compatibility crashes
      '@opentelemetry/instrumentation-undici': { enabled: false }, // Disable to prevent native fetch/undici compatibility crashes
...
if (useExternalExport) {
  ...
  console.log('OTel initialized. Export: Honeycomb OTLP');
} else {
  console.log('OTel disabled in local development to prevent native fetch conflicts.');
}
