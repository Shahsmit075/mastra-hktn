...
router.post('/:id/approve',
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!state.rows.length || !state.rows[0].result_json?.runId) {
        res.status(404).json({ error: 'Workflow run not found for this incident' });
        return;
      }
      ...
      res.status(500).json({ error: 'Failed to resume workflow', details: err.message || JSON.stringify(err) });
    }
  }
);
